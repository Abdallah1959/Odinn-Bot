...
class MovieSelect(discord.ui.Select):
    def __init__(self, results, bot):
        ...
        options = []
        # جلب أفضل 5 نتائج فقط لضمان نظافة القائمة واستقرار الـ UI
        for m in results[:10]:
            ...
            # [تطبيق الإصلاح]: تأمين الوصف وقصه عند 80 حرفًا لمنع أخطاء تجاوز أحرف المنيو في ديسكورد
    ...
    async def callback(self, interaction: discord.Interaction):
        # تأجيل الاستجابة مؤقتاً لجلب بيانات الـ ID الجديد
        await interaction.response.defer(thinking=False)
        movie_id = int(self.values[0])
        
        movie = await self.bot.services.tmdb.get_movie_details(movie_id)
        if not movie:
            await interaction.followup.send("⚠️ حدث خطأ أثناء سحب تفاصيل الفيلم.", ephemeral=True)
            return
        ...
        # تحديث رسالة البوت الأصلية بالـ Embed الجديد والأزرار المطابقة
